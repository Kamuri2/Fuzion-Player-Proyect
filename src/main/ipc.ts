...
async function extractFfmpegMetadata(filePath: string): Promise<any> {
  try {
    ...
    if (finalArtist && finalTitle && finalArtist.includes(' - ')) {
      ...
      if (
        parts[0].toLowerCase().trim() === finalTitle.toLowerCase().trim() ||
      ...
      if (
        !finalArtist ||
        finalArtist === 'Desconocido' ||
        finalArtist === finalTitle ||
        finalArtist.toLowerCase().trim() === parts[0].toLowerCase().trim()
      ) {
        finalArtist = parts[0].trim()
        finalTitle = parts.slice(1).join(' - ').trim()
      }
...
export function setupIpc() {
        ...
        if (rawBuffer && !coverBase64) {
          try {
            if (!hq) {
              ...
              rawBuffer = image
                .resize({ width: 400, height: 400, quality: 'good' })
                .toJPEG(85) as any
            }
            ...
            if (Array.isArray(tags)) {
              ...
              if (lyricsTag && lyricsTag.value) {
                if (typeof lyricsTag.value === 'string') {
                  ...
                  Array.isArray(lyricsTag.value) &&
                  typeof lyricsTag.value[0] === 'string'
                ) {
  ...
  ipcMain.handle('api:getArtistImage', async (_, artistName: string, sampleSongPath?: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://itunes.apple.com/search?term=${encodeURIComponent(artistName)}&entity=musicTrack&limit=5`
      )
      if (response.ok) {
        const data = await response.json()
        if (data && data.results && data.results.length > 0) {
          ...
          let bestResult =
            data.results.find(
              (r: any) => (r.artistName || '').toLowerCase() === (artistName || '').toLowerCase()
            ) || data.results[0]
  ...
  ipcMain.handle(
    'api:translateLyrics',
    async (event, songId: string, lines: string[], targetLang: string = 'es') => {
  ...
  ipcMain.handle(
    'api:getAlbumInfo',
    async (_, artistName: string, albumName: string, targetLang: string = 'es') => {
      try {
  ...
  ipcMain.handle(
    'api:translateUI',
    async (event, langCode: string, baseDictionary: Record<string, any>) => {
      const translationsDir = path.join(app.getPath('userData'), 'ui_translations')
      try {
        if (!existsSync(translationsDir)) {
          await fs.mkdir(translationsDir, { recursive: true })
    ...
    for (const dirent of list) {
      ...
      if (dirent.isDirectory()) {
        ...
        if (
          ...
          if (
            (!hasLyrics || album === 'Desconocido' || artist === 'Desconocido') &&
            (ext === '.opus' || ext === '.ogg')
          ) {
            try {
          ...
          if (
            ...
            artist.toLowerCase() !== folderName.toLowerCase()
          ) {
            const temp = title
            title = artist
            artist = temp
          ...
          results.push({
            ...
            uri:
              'file:///' +
              encodeURI(fullPath.replace(/\\/g, '/')).replace(/#/g, '%23').replace(/\?/g, '%3F'),
