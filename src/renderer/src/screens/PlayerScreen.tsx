    ...
    <div className="flex flex-col h-screen relative overflow-hidden bg-[#121212] select-none">
      ...
      <div className="flex-1 flex flex-row relative z-10 min-h-0 w-full">
        ...
        <div className="flex-1 flex flex-col min-w-0 h-full relative">
          ...
          <div className={`relative z-10 flex-1 min-h-0 flex flex-row items-center justify-center px-8 w-full mx-auto transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] pointer-events-auto ${isFullMode ? 'gap-8' : 'gap-0'}`}>
            ...
            {/* Lyrics Area */}
            <div
              className={`flex flex-col items-center justify-center overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] h-full ${isFullMode
                ? `absolute inset-0 z-10 ${showLyrics ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-8 scale-95 pointer-events-none'}`
                : (showLyrics ? 'relative w-[52%] flex-none opacity-100 translate-x-0 scale-100' : 'relative w-0 opacity-0 flex-none translate-x-12 scale-95')
                }`}
            >
            ...
            {/* Queue Area */}
            <div
              className={`flex flex-col items-center justify-center overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] ${isFullMode
          ...
          <div className={`relative z-10 w-full mx-auto px-8 flex flex-col mt-8 transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] ${showLyrics && !isFullMode ? 'pb-4' : 'max-w-4xl pb-8'}`}>
            <div className={`flex flex-col transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] ${showLyrics && !isFullMode ? 'w-[48%] pl-4 lg:pl-12' : 'w-full'}`}>
              ...
              {/* Playback Controls — hidden when lyrics mode is active */}
              <AnimatePresence>
                {!showLyrics && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, scaleX: 0, height: 0 }}
                    transition={{ duration: 0.7, ease: [0.32, 0.72, 0, 1] }}
                    className="flex flex-row items-center justify-center gap-6 md:gap-10 w-full"
                  >
                    <button
