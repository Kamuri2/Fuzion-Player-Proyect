...
export default function SettingsScreen() {
  ...
  const { colors, isDarkMode, toggleTheme, themeFamily, setThemeFamily, particles, setParticles, customFont, setCustomFont, mascots, addMascot, removeMascot, lyricsFontSize, setLyricsFontSize, showTranslatedLyrics, setShowTranslatedLyrics, lyricsLanguage, setLyricsLanguage, isFullMode, setIsFullMode, albumZenMode, setAlbumZenMode } = useTheme();
    ...
    <div className="flex-1 min-h-screen px-6 py-8 max-w-4xl mx-auto w-full">
      ...
      <div className="space-y-6">
        <div className={`p-6 rounded-2xl border border-white/10 shadow-sm backdrop-blur-2xl ${isDarkMode ? 'bg-black/40' : 'bg-white/40'}`}>
          ...
          <div className="flex flex-row items-center justify-between mb-6">
            <div className="flex flex-col pr-4">
              <span className="font-medium" style={{ color: colors.text }}>{t('settings.classicMode')}</span>
              <span className="text-xs opacity-70 mt-1" style={{ color: colors.subText }}>{t('settings.classicModeDesc')}</span>
            </div>
            <button
              className={`relative inline-flex h-6 w-11 shrink-0 items-center rounded-full transition-colors focus:outline-none ${albumZenMode ? 'bg-green-500' : 'bg-gray-400'}`}
              style={{ backgroundColor: albumZenMode ? colors.primary : undefined }}
              onClick={() => {
                const newValue = !albumZenMode;
                setAlbumZenMode(newValue);
                if (newValue) {
                  navigate('/albums');
                }
              }}
            >
              <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${albumZenMode ? 'translate-x-6' : 'translate-x-1'}`} />
            </button>
          </div>
